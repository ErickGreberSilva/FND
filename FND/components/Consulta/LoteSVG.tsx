...
function toSvgPathFromRings(
  ...
  // 2) Definir área interna (após padding)
  const innerW = Math.max(width - padding * 2, 1);
  const innerH = Math.max(height - padding * 2, 1);

  // 3) Escala uniforme (preserva proporção)
  const scale = Math.min(innerW / spanX, innerH / spanY);

  // 4) Offsets para centralizar
  const scaledW = spanX * scale;
  const scaledH = spanY * scale;
  const offsetX = (width - scaledW) / 2;
  const offsetY = (height - scaledH) / 2;

  // 5) Converter pontos para coordenadas SVG (invertendo Y: lat cresce pra cima, SVG pra baixo)
  ...
  // 6) Gerar path (usa evenodd pra suportar furos)
  let d = "";
  for (const ring of rings) {
...
export default function LoteSVG({
  ...
  className?: string;
  svgSize?: number; // tamanho do canvas do SVG (px)
}) {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setLoading(true);
      try {
        // Consulta a camada de Lotes (16). Retorna geometria em WGS84 (4326)
